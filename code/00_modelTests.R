...
for(k in 1:length(response_vars)) {
  ...
  # run models
  mods <- dir("code/mods", ".stan", full.names=T)
  mods <- setNames(mods, str_sub(dir("code/mods", ".stan"), 3L, -6L))[c(7,8)]
  out <- map(mods, ~stan(., data=d.ls, chains=3, iter=2500, warmup=2000,
                         control=list(adapt_delta=0.95)))
  
  # compare models
  loo_comp <- loo::loo_compare(map(out, loo::loo)); loo_comp
  best_mod <- rownames(loo_comp)[2]
  write.csv(as.data.frame(loo_comp),
            paste0("eda/loo_", Y_var, ifelse(std, "_std", ""), ".csv"))
  saveRDS(out[[best_mod]], paste0("eda/0_out_", Y_var, ifelse(std, "_std", ""), ".rds"))
  ...
  out.clny <- clny.df_i %>% ungroup %>%
    ...
    bind_cols(map_dfc(setNames(pars, pars), 
                      ~summary(out[[best_mod]], pars=.x)$summary[,1])) %>%
    bind_cols(map_dfc(setNames(pars, paste0(pars, "_sd")), 
                      ~summary(out[[best_mod]], pars=.x)$summary[,3]))
  out.wkr <- wkr.df_i %>% ungroup %>%
    full_join(select(out.clny, clny_id, all_of(pars)), by="clny_id") %>%
    mutate(y_pred=summary(out[[best_mod]], pars="y_pred")$summary[,1],
           y_pred_=summary(out[[best_mod]], pars="y_pred_")$summary[,1])
  out_beta <- summary(out[[best_mod]], pars="beta", probs)$summary %>%
    as.data.frame %>% mutate(X_var=c("intercept", names(X_vars_mn)))
  names(out_beta)[3+(1:length(probs))] <- prob_names
  out_alpha <- summary(out[[best_mod]], pars="alpha", probs)$summary %>%
    as.data.frame %>% mutate(X_var=c("intercept", names(X_vars_sd)))
  names(out_alpha)[3+(1:length(probs))] <- prob_names
  ...
  if(any(grepl("^a_mn", names(out[[best_mod]])))) {
    out_a <- summary(out[[best_mod]], pars="a_mn", probs=probs)$summary
    n_a_pars <- nrow(out_a)/d.ls$S
    data.frame(a=out_a[,1], 
               a025=out_a[,5],
               a975=out_a[,11],
               spp=rep(unique(clny.df_i$SPECIESID), n_a_pars),
               par=rep(1:n_a_pars, each=d.ls$S), 
               parName=rep(colnames(d.ls$x_sd)[1:n_a_pars], each=d.ls$S)) %>%
      ggplot(aes(x=a, y=spp)) + 
      geom_vline(xintercept=0, colour="gray90") + 
      geom_point(alpha=0.9, size=2, shape=1) + 
      geom_linerange(aes(xmin=a025, xmax=a975)) +
  ...
  if(any(grepl("^b_mn", names(out[[best_mod]])))) {
    out_b <- summary(out[[best_mod]], pars="b_mn", probs=probs)$summary
    n_b_pars <- nrow(out_b)/d.ls$S
    data.frame(b=out_b[,1], 
               b025=out_b[,5],
               b975=out_b[,11],
               spp=rep(unique(clny.df_i$SPECIESID), n_b_pars),
               par=rep(1:n_b_pars, each=d.ls$S), 
               parName=rep(colnames(d.ls$x_mn)[1:n_b_pars], each=d.ls$S)) %>%
      ggplot(aes(x=b, y=spp)) + 
      geom_vline(xintercept=0, colour="gray90") + 
      geom_point(alpha=0.9, size=2, shape=1) + 
      geom_linerange(aes(xmin=b025, xmax=b975)) +
      theme(panel.grid.major.x=element_blank()) + 
      facet_wrap(~parName, scales="free_x") +
  ...
  ggplot(out.clny, aes_string(quote(mu), Y_var)) + 
  ...
  ggsave(paste0("eda/mu_obs_mn_", f.i), width=5.25, height=5)
  ggplot(out.clny, aes_string(quote(delta), paste0("log(", Y_var, "_sd)"))) +
    geom_abline(linetype=3, size=0.5) + geom_point() + 
    stat_smooth(method="lm", formula=y~x, colour="black", size=0.3) +
    labs(x=expression(delta), y="Observed within-colony log(sd)", title=Y_var)
  ggsave(paste0("eda/mod_obs_sd_", f.i), width=5.25, height=5)
  
  
  
  ## across elevation
  ggplot(out.clny, aes(El_m, mu)) + geom_point() + 
    geom_point(aes(y=y_bar), shape=1, alpha=0.5) +
    {if(std) stat_smooth(method="lm", formula=y~x, colour="gray40", se=F) } +
  ...
  ggplot(out.clny, aes(El_m, delta_exp)) + geom_point() + 
    geom_point(aes(y=d), shape=1, alpha=0.5) +
    {if(std) stat_smooth(method="lm", formula=y~x, colour="gray40", se=F) } +
    {if(!std) stat_smooth(aes(group=SPECIESID), method="lm", formula=y~x,
                          colour="gray40", size=0.3, se=F) } +
    labs(x="Elevation (m)", y="Within-colony sd", title=Y_var)
  ...
  for(j in 1:length(X_vars_mn)) {
    i <- X_vars_mn[j]
    mn_plots[[j]] <- ggplot(out.wkr, aes_string(i, quote(mu))) +
  ...
  for(j in 1:length(X_vars_sd)) {
    i <- X_vars_sd[j]
    sd_plots[[j]] <- ggplot(out.clny, aes_string(i, quote(delta_exp))) +
      geom_point(colour="#2171b5") +
      geom_point(aes_string(y=paste0(Y_var, "_sd")), shape=1, size=0.3, alpha=0.5) +
      stat_smooth(aes(group=SPECIESID), method="lm", formula=y~x,
                  colour="gray40", size=0.3, se=F) +
      labs(x=i, y="") 
  }
  ...
  wkr.post <- map(wkr.sample, 
               ~rstan::extract(out[[best_mod]], pars=paste0("y_pred[", .x, "]")) %>%
