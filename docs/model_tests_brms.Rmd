...
```{r model_setup, eval=F, message=FALSE, warning=FALSE}
...
wkr.base <- trts$wkr.wide %>% 
  ...
  select(Worker, TubeNo, GENUSID, SPECIESID, SampleDate, mnt25,
         any_of(response_vars), any_of(X.ls$all)) 
if(std_method==1) {
  wkr.df <- wkr.base %>% 
    mutate(across(any_of(response_vars, X.ls$all), ~c(scale(.))))
} else if(std_method==2) {
  wkr.df <- wkr.base %>% group_by(SPECIESID) %>%
    mutate(across(any_of(response_vars, X.ls$all), ~c(scale(.))))
} else if(std_method==3) {
  wkr.df <- wkr.base %>% 
    mutate(across(any_of(X.ls$all), ~c(scale(.)))) %>%
    group_by(SPECIESID) %>%
...
for(k in 1:length(response_vars)) {
  ...
  ggsave(paste0(fig_dir, "cond_eff_", Y_var, std_ext, ".pdf"), 
  ...
  p <- map_dfr(mod.ls, ~ .x %>% 
    ...
    facet_grid(.~dpar)
  ggsave(paste0(fig_dir, "b_eff_", Y_var, std_ext, ".pdf"), p, 
         width=8, height=6)
  ...
  p <- map_dfr(mod.ls, ~ .x %>% 
    ...
    labs(x="value", y="", title=Y_var) + 
    facet_grid(.~dpar)
  ggsave(paste0(fig_dir, "b_dens_eff_", Y_var, std_ext, ".pdf"), p, 
         width=8, height=6)
  ...
  ggsave(paste0(fig_dir, "el_mu_", Y_var, std_ext, ".pdf"), p, 
         width=9, height=9)
  ...
  p <- ggplot(post, aes(x=el_m, y=sigma)) + 
    ...
    theme(legend.position=c(0.9, 0.08), 
          legend.key.height=unit(0.1, "cm"))
  ggsave(paste0(fig_dir, "el_sd_", Y_var, std_ext, ".pdf"), p, 
         width=9, height=9)
  ...
  p <- post %>% group_by(TubeNo) %>% sample_n(1e3) %>%
    ...
    theme(legend.position=c(0.9, 0.08), 
          legend.key.height=unit(0.1, "cm"))
  ggsave(paste0(fig_dir, "el_y_", Y_var, std_ext, ".pdf"), p, 
         width=9, height=9)
}
```
