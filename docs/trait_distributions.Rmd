...
clny_all.df <- wkr_all.df %>% group_by(TubeNo, SPECIESID, Trait) %>%
...
```{r box_y, echo=F, message=F, warning=F, fig.height=7}
wkr_dens %>%
  mutate(SPECIESID=factor(SPECIESID, 
                          levels=rev(sort(unique(clny_all.df$SPECIESID))))) %>%
  ggplot(aes(Value, SPECIESID)) + 
  geom_boxplot(aes(fill=GENUSID), outlier.shape=1, outlier.size=0.1, size=0.25) +
  scale_fill_brewer("", type="qual", palette=2) + 
  facet_wrap(~Trait, nrow=2, labeller=label_wrap_gen(15), scales="free_x") +
  labs(x=expression('Value:'~~y[ijs]), y="", title="Worker traits") + 
  theme(legend.position=c(0.9, 0.25),
        panel.grid.major.y=element_line(colour="gray80", size=0.2)) +
  spp_fonts
...
```{r box_CV, echo=F, message=F, warning=F, fig.height=7}
clny_all.df %>% 
  filter(Trait %in% focus_traits) %>%
  mutate(GENUSID=str_sub(SPECIESID, 1, 4),
         Trait=factor(Trait, levels=focus_traits),
...
```{r scatter_CV2, echo=F, message=F, warning=F, fig.height=8}
clny_all.df %>% 
  filter(Trait %in% focus_traits) %>%
  mutate(GENUSID=str_sub(SPECIESID, 1, 4),
         Trait=factor(Trait, levels=focus_traits)) %>%
  left_join(., sp_CV, by=c("SPECIESID", "GENUSID", "Trait")) %>%
  ggplot(aes((sdValue/mnValue)/CV)) + 
  geom_density(aes(group=SPECIESID, colour=GENUSID)) +
  geom_vline(xintercept=1, size=0.25, colour="gray30") +
  scale_colour_brewer("", type="qual", palette=2) + 
  facet_grid(GENUSID~Trait, labeller=label_wrap_gen(15), 
             scales="free_y", space="free_y") +
  labs(x='Intra-colony CV : Species-level CV', 
       title="Species-level CV vs. intra-colony CV") +
...
for(i in 1:n_distinct(complete.df$GENUSID)) {
  ...
  pca_gen[[i]] <- ggplot(complete_i, aes(x=PC1, y=PC2)) + 
    ...
    guides(fill=guide_legend(ncol=4, nrow=4), 
           colour=guide_legend(ncol=4, nrow=4)) +
